...
if "import os" not in src:
    src = src.replace("import re", "import os" + chr(10) + "import re", 1)
nl = chr(10)
bs = chr(92)
dq = chr(34)
new_cfg = ("_ARA = os.path.isfile(r" + dq + "C:" + bs*2 + "Program Files" + bs*2 + "Tesseract-OCR" + bs*2 + "tessdata" + bs*2 + "ara.traineddata" + dq + ")" + nl + "_LANG = " + dq + "ara+eng" + dq + " if _ARA else " + dq + "eng" + dq + nl + "_OCR_CONFIG = f" + dq + "--oem 3 --psm 6 -l {_LANG}" + dq)
src = re.sub(r"_OCR_CONFIG\s*=\s*\([^)]+\)", lambda m: new_cfg, src, count=1, flags=re.DOTALL)
old = ("    confidences = [" + nl + "        int(c) for c in data[" + dq + "conf" + dq + "]" + nl + "        if str(c).lstrip(" + dq + "-" + dq + ").isdigit() and int(c) >= 0" + nl + "    ]" + nl + "    mean_conf = (sum(confidences) / len(confidences) / 100.0) if confidences else 0.0" + nl + "    text = pytesseract.image_to_string(pil_img, config=_OCR_CONFIG)" + nl + "    return text.strip(), round(mean_conf, 3)")
...
if old in src:
    ...
    print("run_ocr patched")
else:
    print("NOT FOUND")
    idx = src.find("confidences = [")
    print(repr(src[max(0,idx-5):idx+300]))
